...
class GRUDecoder(nn.Module):
    def __init__(
        ...
        # Define day classifier
        feat_dim = hidden_dim * (2 if bidirectional else 1)
        self.day_classifier = nn.Sequential(
            nn.Linear(feat_dim, hidden_dim // 2),
            nn.ReLU(),
            nn.Linear(hidden_dim // 2, nDays),
        )
    ...
    def forward(self, neuralInput, dayIdx):
        ...
        rev = grad_reverse(hid, lambd=1.0) 
        day_logits = self.day_classifier(rev)
        return seq_out, day_logits, hid
        # return seq_out
